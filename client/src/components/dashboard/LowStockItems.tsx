  ...
  return (
    <div className="w-full">
      <ul className="divide-y divide-gray-200 dark:divide-white/10">
        {items.map((item) => (
          <li key={item.id} className="py-3 flex items-center justify-between hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors duration-200">
            <div className="flex-1">
              <p className="font-normal text-gray-900 dark:text-white">{item.name}</p>
              <div className="flex items-center mt-1.5">
                <div className="w-full max-w-[120px] bg-gray-200 dark:bg-white/10 h-1">
                  <div 
                    className={`${getBarColor(item.percentRemaining)} h-1`} 
                    style={{ width: `${item.percentRemaining}%` }}
                  ></div>
                </div>
                <span className="text-xs font-light text-gray-500 dark:text-gray-400 ml-2 font-mono">{item.percentRemaining}% left</span>
              </div>
            </div>
            <button 
